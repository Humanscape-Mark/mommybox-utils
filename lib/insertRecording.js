...
async function uploadS3(fileId, barcode) {
  try {
    const filelist = (await fs.readdir(path.join(process.env.RECORDING_FILE_PATH))).filter((filename) => filename.startsWith(fileId));
